			 ...
			 $('body').mousemove(function(e){
			 	if($('body.mobile').length == 0) {
					 	...
					 	if(e.clientX >= $buttonsOffset.left + $containerMargin && 
					 	   ...
					 	   e.clientX <= $logoOffset.left + $containerMargin + $('#header-outer #logo img:visible').width() &&
					 	   e.clientY >= $logoOffset.top && 
					 	   e.clientY <= $logoOffset.top + $logoMargin + $headerOffset.top + $('#header-outer #logo img:visible').height() ) {
...
if($('.nectar-box-roll').length == 0) $(window).load(pageLoadHash);
	...
	//portfolio colors
	if($('.portfolio-items .col .style-3-alt').length > 0 || $('.portfolio-items .col .style-3').length > 0) {
