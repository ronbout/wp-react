 ...
 *
 * *** this is a transpiled version for IE...ugh
 *
 * Ron Boutilier
 * 01/07/2019
 */
...
jQuery(document).ready(function() {
  ...
  var $contWrap = jQuery("div.container-wrap");
  var $footer = jQuery("div#footer-outer");

  function setContWrapPadding() {
    //console.log($contWrap.css("padding-bottom"));
    var footHeight = $footer.height() + "px";
...
function setupRequiredValidation(formName, fieldName) {
  var testFn =
...
function setupValidation(formName, fieldName, validTest) {
  var typeTest =
    ...
    $fields.each(function(i, el) {
      ...
      if (!validTest(fieldVal)) {
        if (!jQuery("#" + errorId).length) {
          ...
          $field.on("input", { field: $field }, function(event) {
            $field = event.data.field;
            var fieldVal = $field.val();
            if (validTest(fieldVal)) {
              var $tdError = jQuery("#" + errorId);
              if ($tdError.length) {
                $tdError.remove();
              }
            }
...
function isEmail(email) {
  ...
  domainSubs.forEach(function(sub) {
