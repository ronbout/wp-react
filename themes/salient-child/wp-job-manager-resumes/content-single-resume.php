 ...
 * 
 * Template override of the Single Resume display because of a
 * bug in the template, referencing fields that have been removed
 * from the form...as it says you can do in the documentation.
 *
...
if ( resume_manager_user_can_view_resume( $post->ID ) ) : ?>
	<div class="single-resume-content">
		...
		<?php if ( ( $skills = wp_get_object_terms( $post->ID, 'resume_skill', array( 'fields' => 'names' ) ) ) && is_array( $skills ) ) : ?>
			<div id="single-resume-skills" class="single-resume-section">
		...
		<?php if ( $items = get_post_meta( $post->ID, '_candidate_education', true ) ) : ?>
			<div id="single-resume-education" class="single-resume-section">
		...
		<?php if ( $items = get_post_meta( $post->ID, '_candidate_experience', true ) ) : ?>
			<div id="single-resume-experience" class="single-resume-section">
				...
				<?php
					foreach( $items as $item ) : ?>
						...
						<?php /* rlb changed the following code to test for a notes field */
		...
		<?php get_job_manager_template( 'contact-details.php', array( 'post' => $post ), 'wp-job-manager-resumes', 
