<?php
/**
 *	Job Alert Entry/Edit form.
 *
 *	This is an override of the WP Job Manager Job Alerts Template
 *
 *	Ron Boutilier
 *	01/04/2018
 */

 /* to keep it simple, I will clear the job alerts dashboard cache anytime
		the user comes to this page.  If site volume reaches a point that this 
		is not sufficient, we will add more specific code */
	pantheon_wp_clear_edge_paths( array('/job-alerts') );
...
<div id="job-alert-container">
	<h2>Job Alert Setup</h2>
	<form method="post" class="job-manager-form" id="job-alert-form">
		<fieldset>
			<label for="alert_name"><?php _e( 'Alert Name', 'wp-job-manager-alerts' ); ?></label>
			<div class="field">
				<input type="text" name="alert_name" value="<?php echo esc_attr( $alert_name ); ?>" id="alert_name" class="input-text" placeholder="<?php _e( 'Enter a name for your alert', 'wp-job-manager-alerts' ); ?>" required />
			</div>
		</fieldset>
		<fieldset>
		...
		<?php endif; ?>
		<fieldset id="job-type-select-fieldset">
