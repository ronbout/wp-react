    ...
    function($) {
        ...
        vc.edit_form_callbacks = [], vc.atts = {
            ...
            parseOne: function(param) {
                var $content, data;
                return $content = this.content(), data = vc.atts.param_group.extractValues.call(this, param, $content), this.$content = $content, encodeURIComponent(JSON.stringify(data))
            },
            init: function(param, $field) {
            ...
            init: function(param, $field) {
                $(".vc_link-build", $field).click(function(e) {
                    ...
                    } : window.wpLink, dialog.open("content"), _.isString(value_object.url) && ($("#wp-link-url").length ? $("#wp-link-url").val(value_object.url) : $("#url-field").val(value_object.url)), _.isString(value_object.title) && ($("#wp-link-text").length ? $("#wp-link-text").val(value_object.title) : $("#link-title-field").val(value_object.title)), $("#wp-link-target").length ? $("#wp-link-target").prop("checked", !_.isEmpty(value_object.target)) : $("#link-target-checkbox").prop("checked", !_.isEmpty(value_object.target)), $("#vc-link-nofollow").length && $("#vc-link-nofollow").prop("checked", !_.isEmpty(value_object.rel)), $vc_link_submit.unbind("click.vcLink").bind("click.vcLink", function(e) {
    ...
    function($) {
        vc.debug = !1, vc.map = _.isUndefined(window.vc_mapper) ? {} : window.vc_mapper, vc.roles = _.isUndefined(window.vc_roles) ? {} : window.vc_roles, vc.Storage = function() {
            ...
            unescapeParam: function(value) {
                return value.replace("`{`", "[").replace("`}`", "]").replace(/(\`{2})/g, '"')
            },
        ...
        vc.shortcode = Backbone.Model.extend({
            ...
            createShortcodeString: function(model, state) {
                ...
                return tag = model.get("shortcode"), params = _.extend({}, model.get("params")), paramsForString = {}, mergedParams = vc.getMergedParams(tag, params), _.each(mergedParams, function(value, key) {
                    ...
                    }, this), mapped = vc.getMapped(tag),
                    isContainer = _.isObject(mapped) && (_.isBoolean(mapped.is_container) && !0 === mapped.is_container || !_.isEmpty(mapped.as_parent)), content = this._getShortcodeContent(model, state), data = {
                        tag: tag,
                        attrs: paramsForString,
                        content: content,
                        type: _.isUndefined(vc.getParamSettings(tag, "content")) && !isContainer ? "single" : ""
            ...
            return paramsMap = vc.getDefaultsAndDependencyMap(tag), outputParams = {}, values = _.extend({}, paramsMap.defaults, values), paramsDependencies = _.extend({}, paramsMap.dependencyMap), _.each(values, function(value, key) {
                if ("content" !== key) {
                    ...
                    if (!_.isUndefined(paramsDependencies[key])) {
                        ...
                        if (dependedElement = paramsDependencies[key].element, dependedValue = values[dependedElement], isDependedEmpty = _.isEmpty(dependedValue), rules = _.omit(paramsDependencies[key], "element"), _.isBoolean(rules.not_empty) && !0 === rules.not_empty && isDependedEmpty || _.isBoolean(rules.is_empty) && !0 === rules.is_empty && !isDependedEmpty || rules.value && !_.intersection(_.isArray(rules.value) ? rules.value : [rules.value], _.isArray(dependedValue) ? dependedValue : [dependedValue]).length || rules.value_not_equal_to && _.intersection(_.isArray(rules.value_not_equal_to) ? rules.value_not_equal_to : [rules.value_not_equal_to], _.isArray(dependedValue) ? dependedValue : [dependedValue]).length) return void(paramsDependencies[key].failed = !0)
                    }
    ...
    function($) {
        vc.saved_custom_css = !1, vc.visualComposerView = Backbone.View.extend({
            ...
            accessPolicyActions: function() {
                ...
                else if ($("#postdivrich").hide(), this.accessPolicy.be_editor) {
                    var _this = this;
                    _.defer(function() {
            ...
            show: function() {
                this.$el.show(), this.$post.hide(), this.$vcStatus.val("true"), this.navOnScroll(), vc.storage.isContentChanged() && (vc.app.setLoading(), vc.app.views = {}, _.defer(function() {
                    vc.shortcodes.fetch({
                        reset: !0
            ...
            close: function() {
                this.$vcStatus.val("false"), this.$el.hide(), _.isObject(window.editorExpand) && _.defer(function() {
                    window.editorExpand.on(), window.editorExpand.on()
                }), this.$post.show(), _.defer(function() {
            ...
            processScroll: function(e) {
                ...
                (!this.navTop || 0 > this.navTop) && this.setNavTop(), this.scrollTop = $(window).scrollTop() + 80, 0 < this.navTop && this.scrollTop >= this.navTop && !this.isFixed ? (this.isFixed = 1, this.$nav.addClass("vc_subnav-fixed")) : this.scrollTop <= this.navTop && this.isFixed && (this.isFixed = 0, this.$nav.removeClass("vc_subnav-fixed"))
            },
        ...
        window.VcRowView = vc.shortcode_view.extend({
            ...
            */
            ...
            render:function () {
                ...
                if(typeof params.id == 'undefined') params.id = (+new Date() + '-' + Math.floor(Math.random() * 11));
